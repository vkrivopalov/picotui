...
**Функциональный паритет и больше** — Picotui предоставляет те же возможности мониторинга кластера, что и Picodata WebUI, включая иерархические представления tiers, replicasets и instances, а также дополнительные функции: гибкая текстовая фильтрация с подсветкой совпадений, которая ищет одновременно по именам инстансов, адресам и failure domains.

**Работает без WebUI** — Picotui подключается напрямую к HTTP API Picodata, поэтому работает с установками Picodata, собранными без `--features webui`. Идеально подходит для минимальных развёртываний или серверов без графического интерфейса.

**Лёгкий и быстрый** — Построен с минимальными зависимостями, использует синхронный ввод-вывод (`ureq` вместо `tokio`/`reqwest`) и легковесный парсинг аргументов (`pico-args` вместо `clap`). Stripped-бинарник занимает всего ~3.5 МБ без runtime-зависимостей кроме libc.
...
Индикаторы Raft voter/leader и состояние replicaset требуют **Picodata 26.2+**.
...
- **Индикаторы Raft-статуса**: Отображение Raft-лидера и голосующих инстансов (Picodata 26.2+)
- **Состояние Replicaset**: Отображение фактического состояния replicaset (ready/not-ready) из метаданных кластера (Picodata 26.2+)
...
| `H` | Показать статус здоровья инстанса (требуется HTTP-адрес) |
...
**Обозначения** (Picodata 26.2+):
- `★` — vshard-лидер (мастер replicaset)
- `⚡` — Raft-лидер (кластерный)
- `V` — Raft voter (голосующий)
...
- Имя replicaset и состояние лидера (Online/Offline/Expelled)
- Индикатор состояния replicaset: `✓` (ready) или `?` (not-ready) — Picodata 26.2+
...
- Имя инстанса с индикаторами ролей:
  - `★` — vshard-лидер (мастер replicaset)
  - `⚡` — Raft-лидер (Picodata 26.2+)
